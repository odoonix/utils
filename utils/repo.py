...
def get_addons_module(repo_path_list:list[str]) -> list[AddonsModel]:
    ...
    for repo_path in repo_path_list:
            ...
            if result:
                ...
                if verson_list and workspace_module and name_module:
                    addons = AddonsModel(workspace=workspace_module, description='Successfully loaded!',name=name_module, versions_odoons=verson_list, is_private=False)
                    addons.status = 1
                    result_version.append(addons)
                else:
                    addons = AddonsModel(workspace=workspace_module, description='.No matching version found.',name=name_module, versions_odoons=['', ''], status=2, is_private=False)
                    result_version.append(addons)                
